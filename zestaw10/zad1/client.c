...
int calculate (arith_request * req);

sa_family_t
get_connection_type (char *arg)
{
...
void
connect_inet (int fd, const char *addr_str)
{
    ...
    *delim = '\0';
    ...
    OK (connect (fd, (const struct sockaddr *) &sockaddr, sizeof (sockaddr)),
        "Error connecting to inet socket");
    fprintf (stderr, "Connected to server via INET\n");
}


void
connect_local (int fd, const char *path)
{
...
send_message (int socket, msg_type type, void *data, size_t len)
{
    ...
    strncpy (msg->client_name, name, MAX_NAME_LEN);
    if (len > 0)
    {
...
do_register (int fd)
{
    ...
    strncpy (buff.client_name, name, MAX_NAME_LEN);
...
do_listen (int fd)
{
    const size_t LEN = sizeof (message) + sizeof (arith_request);
    ...
    while (true)
    {
        ...
        switch (buff->type)
        {
            ...
            case ARITH:
                ...
                send_message (fd, RESULT, &resp, sizeof (arith_response));
...
main (int argc, char *argv[])
{
    ...
    sa_family_t family = get_connection_type (argv[2]);

    socketfd = socket (family, SOCK_STREAM, 0);;
    OK (socketfd, "Error opening socket") _socket_fd = socketfd;
