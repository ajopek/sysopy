...
connect_local (int fd, const char *path)
{
    ...
    OK (bind (fd, (const struct sockaddr *) &sockaddr, sizeof (sa_family_t)),
        "Error binding to local socket");
    OK (connect (fd, (const struct sockaddr *) &sockaddr, sizeof (sockaddr)),
        "Error connecting to local socket");
...
main (int argc, char *argv[])
{
    ...
    socketfd = socket (family, SOCK_DGRAM, 0);;
