...
void handle_message(const message *msg, const conn *sender);
...
void send_message (const conn * addr, msg_type type, void *data,
                   uint32_t len);
...
message *
read_message (int socket, conn * sender)
{
    ssize_t bytes;
    ...
    bytes =
            recvfrom (socket, buff, sizeof (message), 0, &sender->addr,
                      &sender->addrlen);
    OK (bytes, "Error receiving message") if (bytes == 0)
    {
...
void
send_message (const conn * addr, msg_type type, void *data, uint32_t len)
{
    ...
    OK (sendto (socket, msg, sizeof (message), 0, &addr->addr, addr->addrlen),
...
void
handle_message(const message *msg, const conn *sender)
{
    switch (msg->type)
    {
        case REGISTER:
...
void
handle_register (const char *name, const conn * sender)
{
...
void
handle_result (const arith_resp * resp, const char *client_name)
{
